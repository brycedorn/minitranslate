...
function update_lib(i) {
  ...
  content = $.map(mt_lib, function(entry,i) {
    return "<tr><td><h4>" + entry.w + "</h4></td><td><span style='padding:8px' class='midarrow glyphicon glyphicon-circle-arrow-right'/></td><td><h4>" + entry.r + "</h4></td><td><h4 align='right'><button class='btn btn-warning' onclick=update_lib("+i+")><a><span class='glyphicon glyphicon-remove'/></a></button></h4></td></tr>";
  });
  $("#library").append(content.join(""));
  $("#library").append("<tr><td><input type='text' class='form-control word-input' placeholder='Add a word'></input></td><td><span style='padding:8px' class='glyphicon glyphicon-circle-arrow-right'/></td><td><input type='text' class='form-control replace-input' placeholder='Add its translation'></input></td><td><h4 align='right'><button class='btn btn-large btn-info add' onclick='add_word($(this))'><a><span class='glyphicon glyphicon-plus'/></a></button></h4></td></tr>");
}
